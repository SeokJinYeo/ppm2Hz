function b0 = nppm2Hz_KD(chi,dr,ff,bf,n,B0)
if nargin < 7
    B0 = 3;
 if nargin < 6
     l = [0 0 1];
...
kernel = 1/3 -( n1^2*f1(KX_SQ,KY_SQ,KZ_SQ)+n2^2*f1(KY_SQ,KX_SQ,KZ_SQ)+n3^2*f1(KZ_SQ,KY_SQ,KX_SQ)+...
    (2*n1*n2)*f2(KX_SQ,KY_SQ,KZ_SQ)+(2*n1*n3)*f2(KX_SQ,KZ_SQ,KY_SQ)+...
    (2*n2*n3)*f2(KZ_SQ,KY_SQ,KX_SQ));
...
DX = tem(ff:ff:end,ff:ff:end,ff:ff:end);    % undersampling from the full matrix
    ...
    function K = f1(KX_SQ,KY_SQ,KZ_SQ)
        K = (KX_SQ.^2)./(KX_SQ.^2 + KY_SQ.^2 + KZ_SQ.^2);
    end
