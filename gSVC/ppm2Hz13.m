function [b0,S,T] = ppm2Hz13(chi,dr,r,t)
...
K = asinh(Y2./sqrt(X2.^2+Z2.^2))+asinh(Y2./sqrt(X1.^2+Z1.^2))+...
    asinh(Y1./sqrt(X1.^2+Z2.^2))+asinh(Y1./sqrt(X2.^2+Z1.^2))-...
    asinh(Y2./sqrt(X2.^2+Z1.^2))-asinh(Y2./sqrt(X1.^2+Z2.^2))-...
    asinh(Y1./sqrt(X2.^2+Z2.^2))-asinh(Y1./sqrt(X1.^2+Z1.^2));  % X-Z


% F = fftn(K)/4/pi;                 % [OLD], source-centered kernel
F = conj(fftn(K))/4/pi;             % [NEW], target-centered kernel

factor = 42.578e6*3.0*1e-6;         % ppm to Hz
